"""Browser-based qrels annotation server."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the qrels annotation web UI.")
    parser.add_argument(
        "--candidates-csv",
        type=Path,
        default=DEFAULT_CANDIDATES_CSV,
        help="Path to review_candidates.csv",
    )
    parser.add_argument(
        "--kpis-csv",
        type=Path,
        default=DEFAULT_KPIS_CSV,
        help="Path to kpis_long.csv for target values",
    )
    ...
    parser.add_argument("--session-name", default="Qrels annotation session")
    parser.add_argument("--annotator", default="anonymous")
    parser.add_argument(
        "--manifest-path",
        type=Path,
        default=None,
        help="Optional prebuilt queue manifest JSON.",
    )
...
def prepare_session(args: argparse.Namespace) -> str:
    ...
    manifest_items, index_summary = resolve_session_source(
...
def cached_pages(mmd_path: str) -> tuple[str, ...]:
    return tuple(load_pages(mmd_path))
...
def create_app(default_session_id: str | None, build_defaults: dict[str, Any]) -> Flask:
    ...
    def api_create_session() -> Any:
        ...
        manifest_path_value = payload.get("manifest_path") or defaults.get("manifest_path")
        manifest_path = Path(manifest_path_value) if manifest_path_value else None
        manifest_items, index_summary = resolve_session_source(
            manifest_path=manifest_path,
            candidates_csv=Path(payload.get("candidates_csv") or defaults["candidates_csv"]),
            kpis_csv=Path(payload.get("kpis_csv") or defaults["kpis_csv"]),
        ...
        config = {
            "candidates_csv": payload.get("candidates_csv") or defaults["candidates_csv"],
            "kpis_csv": payload.get("kpis_csv") or defaults["kpis_csv"],
        ...
        metadata = create_session(
            session_name=str(payload.get("session_name") or "Qrels annotation session"),
    ...
    def api_raw_image(session_id: str, index: int) -> Any:
        ...
        raw_root = Path(item.get("raw_root") or str(DEFAULT_RAW_ROOT)).resolve()
    ...
    def api_inline_image(session_id: str, index: int, rel_path: str) -> Any:
        item = get_item_or_404(session_id, index)
        report_dir = Path(item["mmd_path"]).parent
...
def main(argv: list[str] | None = None) -> int:
    ...
    elif args.annotator != "anonymous" or args.session_name != "Qrels annotation session":
        session_id = prepare_session(args)
